::page{title="Deploying Microservices "}

<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0250EN-SkillsNetwork/images/IDSN-logo.png" width="200" alt="cognitiveclass.ai logo"><br>
...
**Option 1: AI-Graded Submission and Evaluation**: Copy and paste the **exact terminal output** showing successful execution of the `ibmcloud ce project current` command and save it in a **text file named `deploy-getpic-1`** for the final project submission and evaluation.

**Option 2: Peer-Graded Submission and Evaluation**: Take a screenshot of the terminal showing the output of the `ibmcloud ce project current` command and save it as `deploy-getpic-1.jpg` (or `.png`).
...
1. You can see both the original and the tagged images by using:
	...
	```
	> Note that the orignal image and the tagged images have the same `IMAGE ID` indicating the difference is in tags only.
...
1. Copy the application URL, paste it in a new tab, and press **Enter**. You will need to add a valid path to see a result. You can test `URL/count` or `URL/health` to test the microservice.
...
**Option 1: AI-Graded Submission and Evaluation**: Copy and save the **exact terminal output** showing successful execution of the `ibmcloud ce app create` command (including the final application URL) in a **text file named `deploy-getpic-2`** for the final project submission and evaluation.

**Option 2: Peer-Graded Submission and Evaluation**: Take a screenshot of the terminal showing the output of the `ibmcloud ce create` command with the final application URL and save it as `deploy-getpic-2.png` (or `.jpg`). Also save the URL of the pictures service for use in the next lab.
...
2. Choose from the left menu and switch to the **Administrator** option.

<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/1.PNG" width="600">

3. Navigate through the options in the left-hand menu to access **Builds**, then proceed to click and open the **imageStreams** section.

<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/2.PNG" width="600">

4. Select the option to generate an ImageStream by clicking the **Create ImangeStream** button button.

<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/3.PNG" width="600">
...
```
<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/4.PNG" width="600">

6. Next, navigate to the **Developer** mode using the options available in the left-hand menu.

<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/5.PNG" width="600">

7. Select the **"Add+"** option by clicking on it.

<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/6.PNG" width="600">

8. Choose **"Container images"** to initiate the deployment of the image generated in the previous step.

<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/7.PNG" width="600">
...
<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/8.PNG" width="600">
...
<img src="https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0320EN-SkillsNetwork/images/9.PNG" width="600">
...
1. Take a screenshot of the **Image** page showing the Container Image as `mongo` and the Tag as `latest`. Save it as `deploy-getsong-1.png` (or `.jpg`).
...
1. Copy and paste the command from the previous output into a new browser, using the URL `http://$URL/health`.You should get a result as follows:
...
> If you get an `Application is not available` error, make sure the URL starts with `http://`, as your browser may redirect you to the `https://` secure version.
	...
	```
	Take a screenshot of output of the hiting the `/health` endpoint. Save the screenshot as `deploy-getsong-5.jpg` (or .png).

## Assessment:

### Option 1: AI-Graded Submission and Evaluation

1. Copy and save the **exact terminal output** showing successful execution of the `oc new-app` command for the Songs microservice in a **text file named `deploy-getsong-3`**.
2. Copy and save the **exact terminal output** showing successful execution of the `oc expose service/songs` command in a **text file named `deploy-getsong-4`**.
3. Take a screenshot of the running application in your browser showing the `/health` endpoint and save it as `deploy-getsong-5.jpg` (or `.png`).

---

### Option 2: Peer-Graded Submission and Evaluation

1. Take a screenshot of the terminal output of the `oc new-app` command for the Songs microservice and save it as `deploy-getsong-3.jpg` (or `.png`).
2. Take a screenshot of the terminal output of the `oc expose svc` command and save it as `deploy-getsong-4.jpg` (or `.png`).
3. Take a screenshot of the running application in your browser showing the `/health` endpoint and save it as `deploy-getsong-5.jpg` (or `.png`).
...
| 2024-04-26 | 0.4 | Manvi Gupta | Instructions updated |
| 2024-12-02 | 0.5 | Manvi Gupta | Instructions updated in last step |
| 2026-19-01 | 0.6 | Nikesh Kumar | Updated instructions as per PR and MARK |
|   |   |   |   |-->
